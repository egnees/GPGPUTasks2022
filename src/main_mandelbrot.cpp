...
int main(int argc, char **argv)
{
    ...
    unsigned int benchmarkingIters = 3;
...
void renderInWindow(float centralX, float centralY, unsigned int iterationsLimit, bool useGPU)
{
    ...
    float newCentralX = centralX;
    float newCentralY = centralY;
    ...
    do {
        ...
        if (started) {
          sizeX /= zoomingSpeed;
          sizeY /= zoomingSpeed;
          centralX += (newCentralX - centralX) * changeFocusSpeed;
          centralY += (newCentralY - centralY) * changeFocusSpeed;
        }
